...
本项目采用 [MIT 许可证](LICENSE.md)，这意味着您可以自由使用、复制、修改、合并、发布、分发、再授权，甚至出售本软件。不过，我们也提供定制、维护和托管服务。我们可以为您提供更高优先级的企业级软件更新，这将极大地促进我们项目的可持续发展。我们也欢迎您为本项目提供资金支持。请联系我们以获取更多详细信息。

*** 重要 *** 3.0.0-final 是最后一个可供开源使用的版本，自 4.0.0 版本起我们将不再提供开源代码，这不意味着YALS不再提供开源更新，而是为了让这个项目更好的发展下去，我们仍然会不定时地基于 3.0.0-final 版本发布有必要的更新，包含有用的插件模块等，我们十分欢迎有使用需求的用户联系我们以获取 4.0.0+ 商业版本的详情与定价。
...
YALS 支持自定义命令插件。将插件可执行文件放置在相应的插件目录中：
- 服务器插件：`./internal/plugin/server/`
- 代理插件：`./internal/plugin/agent/`

插件应接受命令行参数，并将结果输出到 stdout/stderr。
