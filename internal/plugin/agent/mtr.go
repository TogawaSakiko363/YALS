...
func (p *MTRPlugin) Execute(target string) (string, error) {
	// Use streaming implementation with buffer to maintain consistency
...
func (p *MTRPlugin) ExecuteStreaming(target string, callback plugin.StreamingCallback) error {
	// Create a context that can be cancelled for consistency with ExecuteStreamingWithID
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	return p.runStreamingMTR(target, callback, ctx)
}
...
func (p *MTRPlugin) ExecuteStreamingWithID(target, commandID string, callback plugin.StreamingCallback) error {
	...
	// Create MTR command with consistent parameters
...
func (p *MTRPlugin) runStreamingMTR(target string, callback plugin.StreamingCallback, ctx context.Context) error {
	...
	// Use consistent packet count (10) for all streaming executions
	cmd := exec.CommandContext(ctx, "mtr", "--raw", "-c", "10", target)
	return p.runStreamingMTRWithContext(ctx, cmd, target, callback)
}
...
func (p *MTRPlugin) runStreamingMTRWithContext(ctx context.Context, cmd *exec.Cmd, target string, callback plugin.StreamingCallback) error {
	...
	case err := <-done:
		...
		// Send final results with completion flag
		output := p.formatMTRResult(result)
...
func (p *MTRPlugin) formatMTRResult(result *MTRResult) string {
	...
	// Used to record the last valid IP address
	...
	for _, ttl := range ttls {
		...
		// [New Logic] Detect duplicate IPs
		// If the IP of the current Hop exists and is the same as the IP of the previous Hop,
		// this means the target has been reached and is continuously echoing, so the output is terminated directly.
		if hop.IP != "" && hop.IP == lastValidIP {
			break
		}

		// Update lastValidIP (only if the current IP is valid).
		if hop.IP != "" {
			lastValidIP = hop.IP
		}
